...
class EGNN(nn.Module):
    def __init__(
        ...
        self.normalization_factor = normalization_factor
        self.aggregation_method = aggregation_method
        self.sperate_charge_net = seperate_charge_net
        ...
        if self.sperate_charge_net:
            self.embedding_out_charge = nn.Linear(self.hidden_nf, 2)
            self.embedding_out_atom = nn.Linear(self.hidden_nf, out_node_nf - 2)
        else:
            self.embedding_out = nn.Linear(self.hidden_nf, out_node_nf)
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None):
        ...
        # Important, the bias of the last linear might be non-zero
        if not self.sperate_charge_net:
            h = self.embedding_out(h)
        else:
            h_charge = self.embedding_out_charge(h)
            h_atom = self.embedding_out_atom(h)
            h = torch.cat([h_atom, h_charge], dim=1)
